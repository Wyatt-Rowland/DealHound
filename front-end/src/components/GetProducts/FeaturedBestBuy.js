...
import useFeaturedBestBuyProducts from './useFeaturedBestBuy';
import { Box, Text, Image, Link, Flex, Badge } from '@chakra-ui/react';

const FeaturedBestBuy = () => {
  const featuredProduct = useFeaturedBestBuyProducts();

  // Check if featuredProduct is not null before trying to access its properties
  if (!featuredProduct) {
    // Render a loading state or placeholder until the featured product is fetched
